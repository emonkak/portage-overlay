...
if [[ ${PV} == *9999* ]]; then
	...
	RELEASE_URI="http://www.splitted-desktop.com/~gbeauchesne/mplayer-vaapi/mplayer-vaapi-${PV#*p}-FULL.tar.bz2"
fi
SRC_URI="${RELEASE_URI}
	!truetype? ( ${FONT_URI} )"
...
S="${WORKDIR}/mplayer-vaapi-${PV#*p}/mplayer-vaapi"
...
src_prepare() {
	...
	if use vaapi; then
		epatch "${S}/../patches/mplayer-vaapi.patch"
		epatch "${S}/../patches/mplayer-vaapi-gma500-workaround.patch"
		epatch "${S}/../patches/mplayer-vaapi-0.29.patch"
		epatch "${S}/../patches/mplayer-vdpau.patch"
	fi
