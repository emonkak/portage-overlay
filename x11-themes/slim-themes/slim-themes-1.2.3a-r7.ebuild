...
src_prepare() {
	...
	THEMES=(
		Zenwalk/slim.theme
		capernoited/slim.theme
		debian-moreblue/slim.theme
		fingerprint/slim.theme
		flat/slim.theme
		flower2/slim.theme
		gentoo/slim.theme
		isolated/slim.theme
		lake/slim.theme
		lotus-midnight/slim.theme
		lotus-sage/slim.theme
		mindlock/slim.theme
		parallel-dimensions/slim.theme
		previous/slim.theme
		rear-window/slim.theme
		slim-archlinux/slim.theme
		slim-gentoo-simple/slim.theme
		slim.theme
		subway/slim.theme
		wave/slim.theme
	)
	for theme in ${THEMES[@]}
	do
		sed -i -e 's|Verdana|Liberation Sans|g' ${theme} || die
	done
}
